...
func _tick():
	...
	if throwbox.active and throwbox.enabled:
		var wb = windbox if not host.initiative else bigger_windbox
		...
		for obj in host.objs_map.values():
			if obj is BaseObj and not obj == host and not obj.is_grounded() and obj.current_state().state_name != "Burst" and wb.overlaps(obj.hurtbox):
