	...
	if not ReplayManager.playback:
		if not is_waiting_on_player():
			...
			if p1.state_interruptable and not p1_turn:
				p2.busy_interrupt = ( not p2.state_interruptable and not (p2.current_state().interruptible_on_opponent_turn or p2.feinting))
				...
				someones_turn = true
				p1.busy_interrupt = ( not p1.state_interruptable and not (p1.current_state().interruptible_on_opponent_turn or p1.feinting))
